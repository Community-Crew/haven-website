...
**Built (added after Part G landed):** `BoardPosition::grantRoleFor()` now itself refuses to grant (locally or in Keycloak) unless `ndaSatisfiedFor()` is true - covering a fresh `MembershipObserver` assignment. `BoardPositionSignatureObserver` (`created()`/`updated()`/`deleting()` on `BoardPositionSignature`) handles the other direction - marking, unmarking, or deleting a signature after the position was already assigned - by looking up whichever currently-open membership holds that exact position (at most one, per the "one open membership at a time" invariant) and re-running `grantRoleFor()`/`revokeRoleFor()` accordingly. So unlike the original plan here, unmarking a signature (or deleting a signed one) *does* revoke the role and Keycloak group membership, not just the permission. `User::hasPermissionViaRole()`'s NDA check (above) remains as a second, independent layer - it still applies if a role is ever assigned some other way, outside `grantRoleFor()`.
...
**Superseded:** a single `board_position_id` column on `memberships` only ever let a user hold *one* board position at a time (the same "one open membership" invariant this section leaned on for a clean 1:1 swap turned out to also be the bug - a user can genuinely hold several simultaneously, e.g. chair of a commission **and** treasurer of the board **and** commissioner of digital affairs). Migration `create_board_position_assignments_table` moved position-holding into its own table, `board_position_assignments` (`user_id`, `board_position_id`, `is_public`, `sort_order`, `ended_at` nullable - set, not deleted, when revoked, so history stays on record), decoupled entirely from `Membership` - the same pattern `board_position_signatures` already used. It backfills any live `memberships.board_position_id` data before dropping those three columns from `memberships`. `BoardPositionAssignmentObserver` now owns the grant/revoke `MembershipObserver` used to do inline; `MembershipObserver` itself just ends every active assignment for a user when their membership stops being open (a user with no open membership isn't an active member, full stop - which position(s) that revokes is no longer its concern). `BoardPositionSignatureObserver` looks up the active assignment via `BoardPositionAssignment` instead of an open `Membership`. Managed via a new `BoardPositionAssignmentResource` (mirrors `BoardPositionSignatureResource`'s create-only-pair pattern, plus mark/unmark-style `end_assignment`/`reinstate` actions) and a `BoardPositionAssignmentsRelationManager` on `UserResource` (mirrors `MembershipsRelationManager`).

## Part H — Public API

`GET /api/v1/board` → new `App\Http\Controllers\Api\Board\BoardIndexController`, unauthenticated, grouped with the other public reads (`/rooms`, `/agendas`) in `routes/api.php`. Query: `BoardPositionAssignment::whereNull('ended_at')->where('is_public', true)->with(['user', 'boardPosition.organisation'])->orderBy('sort_order')->orderBy('created_at')->get()` (updated from the original `Membership`-based query above once position-holding moved to its own table), mapped through a small `App\Http\Resources\Api\BoardMemberResource` (`name`, `title` = position name, `organisation` name for commission board members). Frontend consumption (an actual public board page in `HavenWebsite-Frontend`) is a separate follow-up, not covered here.
